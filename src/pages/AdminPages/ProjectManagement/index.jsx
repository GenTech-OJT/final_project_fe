...
const ProjectManagement = () => {
  ...
  const toggleStatus = record => {
    ...
    const updatedGridData = gridData.map(item => {
      if (item.id === record.id) {
        return {
          ...item,
          status: item.status === 'In process' ? 'Not Asign' : 'In process',
        }
      }
      return item
    })
    ...
    record.status === 'active'
      ? showToast(t('inProcess_successfully'), 'success')
      : showToast(t('notAsign_successfully'), 'success')
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (_, record) => (
        <Button
          type={record.status === 'In process' ? 'primary' : 'danger'}
          onClick={() => toggleStatus(record)}
          style={{
            backgroundColor:
              record.status === 'In process' ? '#1890ff' : '#ff4d4f',
            borderColor: 'transparent',
            color: 'white',
          }}
        >
          {record.status === 'In process' ? t('inProcess') : t('notAsign')}
        </Button>
      ),
    },
    {
